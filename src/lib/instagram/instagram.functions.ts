...
export const getFacebookBusinessAuthorizeUrl = createServerFn({ method: "GET" }).handler(
  async (): Promise<{ url: string | null; configured: boolean; error?: string }> => {
    ...
    if (!appId || !configId || !redirectUri || !getEnv("META_APP_SECRET")) {
      return {
        url: null,
        configured: false,
        error: "META_APP_ID, META_FACEBOOK_CONFIG_ID, META_FACEBOOK_REDIRECT_URI और META_APP_SECRET configure करना बाकी है।",
      };
    }
...
export const verifyConfiguredInstagramToken = createServerFn({ method: "GET" }).handler(
  async (): Promise<{ configured: boolean; valid: boolean; userId?: string; username?: string; error?: string }> => {
    ...
    try {
      ...
      return { configured: true, valid: false, error: error instanceof Error ? error.message : "Instagram API request failed." };
    }
  },
);
  ...
  async (): Promise<{
    ...
    try {
      ...
      return {
        ...
        reelsCount: media.filter((item) => item.media_type === "VIDEO" || item.media_product_type === "REELS").length,
        latestTimestamp: media[0]?.timestamp,
      };
    } catch (error) {
      return { configured: true, valid: false, mediaEndpoint: "failed", error: error instanceof Error ? error.message : "Instagram media API request failed." };
    }
  },
);
...
export const exchangeFacebookBusinessCode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ pageId: string; pageName?: string; instagramUserId: string; instagramUsername?: string }> => {
    ...
    if (!appId || !appSecret || !redirectUri || !supabaseUrl || !serviceRoleKey) {
      throw new Error("Meta OAuth or Supabase server configuration is incomplete.");
    }
    ...
    if (!page?.id || !page.access_token || !page.instagram_business_account?.id) {
      throw new Error("कोई ऐसा Facebook Page नहीं मिला जो Professional Instagram Account से linked हो।");
    }
    ...
    const saveResponse = await fetch(`${supabaseUrl}/rest/v1/instagram_meta_connections?on_conflict=page_id,instagram_user_id`, {
      ...
      body: JSON.stringify({
        ...
        instagram_user_id: igPayload.id,
        instagram_username: igPayload.username ?? null,
